...
parser.add_argument('--image_mode', type=str, choices=['Euclid', 'Roman'], default='Euclid', help='Whether to generate images in Euclid or Roman mode.')
args = parser.parse_args()

NUM_IMAGES_PER_BASE = int(args.num_images)
...
for base_class in base_classes:
    ...
    inputs_root = './inputs'
    inputs_prefix = f'{args.image_mode}_{base_class}_{args.output_dir.split("/")[-1].split("_")[-1]}'
    ...
    os.system(f'python create_images_real.py --base_class {base_class} --input_path {inputs_root} -o {args.output_dir} --edge_galaxy {inputs_prefix}_additional.csv --deflector_params {inputs_prefix}_deflector.csv --source_params {inputs_prefix}_source.csv --image_mode {args.image_mode}')